        ...
        static class Consumer extends BaseThread
        {
                 ...
                 public void run()
                 {
                              ...
                              for (int i = 0; i < StackManager.iThreadSteps; i++)  
                              {
                                     ...
                                     // ********************
                            	  try {
                            		  consumer.Wait(this.iTID);
                            		  mutex.Wait(this.iTID);
                            		  this.copy = stack.pop();//pop top item out of stack
          ...
          * Inner class Producer
          ...
          static class Producer extends BaseThread
          {
                     ...
                     public static int runtimes;// time of producer thread pushed
                     ...
                     public void run()
                     {
                                ...
                                for (int i = 0; i < StackManager.iThreadSteps; i++)
                                {
                                	...
                                	try {   
                                		mutex.Wait(this.iTID);
                                		char top = stack.pick();//check character on the top
                                		this.block = (char)(top + 1);//get next character to push in 
                                		stack.push(this.block);//push it into charstack
                                	
                                	} catch (CharStackEmptyException ee) {
                                		this.block = 'a';//stack is empty, nothing to pick, we need assign a character to push in 
                                		try {
                                		...
                                		System.out.println("Producer thread [TID=" + this.iTID + "] pushes character =" + this.block);
                                		 ...
                                		 this.runtimes ++;
                                         if (this.runtimes == 2 * StackManager.iThreadSteps ) {
                                        	 mutex.Signal();
                                      	     consumer.Signal();
                                         }else {
                                        	 mutex.Signal();
           ...
           static class CharStackProber extends BaseThread
           {
                     public void run()
                     {
                               ...
                               for (int i = 0; i < 2 * StackManager.iThreadSteps; i++)
                               {
                            	   ...
                            	   } catch (CharStackInvalidAceessException iae) {
                            		   iae.printStackTrace();
                            	   } finally {
                            	       mutex.Signal();
                            	   }
